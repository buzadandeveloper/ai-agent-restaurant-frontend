...
export const getRestaurantDialogSchema = (isEditMode: boolean) => {
  ...
  const menuCsvSchema = isEditMode
    ? z.instanceof(File).optional().or(z.literal(null))
    : z
        .instanceof(File, { message: "File is required." })
        .refine((file) => file.size <= MAX_UPLOAD_SIZE, {
          message: "File size must be less than 3MB."
